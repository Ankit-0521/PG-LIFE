...
def search():
    ...
    if word in data:
        ...
        best_match=get_close_matches(word,data.keys())
        result=messagebox.askyesno('Confirm', f'Did yoy mean {best_match[0]} instead?')

        if result==True:
            entryField.delete(0,END)
            entryField.insert(0,best_match[0])
            textarea.config(state=NORMAL)
            meaning=data[best_match[0]]
            for m in meaning:
                textarea.insert(END,u'\u2022'+m+'\n\n')
            textarea.config(state=DISABLED)

        else:
            closest_matches=get_close_matches(word, data.keys())[1]
            result = messagebox.askyesno('Confirm', 'Did yoy mean ' + closest_matches[1] + ' instead?')

            if result == True:
                entryField.delete(0, END)
                entryField.insert(0, closest_matches[1])

                meaning = data[closest_matches[1]]
                textarea.config(state=NORMAL)
                for m in meaning:
                    textarea.insert(END, u'\u2022' + m + '\n\n')
                ...
                messagebox.showerror('Error', 'Please double check the word, It does not exist.')
...
root=Tk()
root.title('Dictionary created by Ankit')
root.geometry('1000x626+10+30')
root.resizable(0,0)
bgImage=PhotoImage(file=r'C:\Users\ankit gupta\OneDrive\Pictures\Saved Pictures\bg.png')
...
textarea=Text(root,font=('Arial',18,'bold'),width=34,height=8,bd=8,relief=GROOVE,wrap='word', state=DISABLED)
